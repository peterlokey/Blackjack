#TODO 
#fix - user can currently double down after hitting!
#make every You Lose statement print the amount lost too ???
...
def double_down(hand, wager, bank):
    if (bank - wager) < wager:
        wager = bank
        print('Not enough to double. New wager = ' + str(bank))
    else: wager = wager * 2
    hand = hit(hand)
    return wager

def split(hand, dealer_hand, wager, bank):
    hand1 = [hand[0], deal()]
    hand2 = [hand[1], deal()]
    print('\nHAND 1')
    result1, wager1 = play_hand(hand1, dealer_hand, wager, bank)
    print('\nHAND 2')
    result2, wager2 = play_hand(hand2, dealer_hand, wager, bank)
    winnings = 0
    if result1 == 'BJ':
        winnings += (wager1 * 1.5)
    if result1 == 'Win':
...
def change_wager(bank, wager):
    ...
    while new_wager < 10 or new_wager > 300:
        try: 
            ...
            print('Wager must be a number! (no decimals)')
...
def play_hand(user_hand, dealer_hand, wager, bank):
    ...
    if is_blackjack(dealer_hand):
        ...
        print('Dealer has Blackjack. You lose '+ str(wager))
        return 'Lose', wager
    ...
    while choice != '2':
        if user_hand[0] == user_hand[1]:    #SPLIT
            ...
            if choice == '4':
                winnings = split(user_hand, dealer_hand, wager, bank)
        ...
        if choice == '1':       #HIT
            ...
            if hand_total(user_hand) > 21:
                print('BUST - You lose '+ str(wager)+'.')
                input('Press Enter to continue...')
                return 'Lose', wager
            choice = 0
        elif choice == '3':     #DOUBLE DOWN
            wager = double_down(user_hand, wager, bank)
            if hand_total(user_hand) > 21:
                print('BUST - You lose '+ str(wager)+'.')
                input('Press Enter to continue...')
                return 'Lose', wager
...
def main_menu(bank, wager):
    ...
    while selection !=1 and selection !=2 and selection !=3:
        if bank <10 :
            print("You don't have enough money. Thanks for playing.")
            input()
            return 3, wager
        ...
        print('3 Quit')
        selection = input()
        if selection == '1': return 1, wager
        elif selection == '2': return 2, wager
        elif selection == '3':return 3, wager
...
def main():
    ...
    user_input = 0
    while user_input != 3:
        user_input, wager = main_menu(bank, wager)
        if user_input == 1: 
            user_hand, dealer_hand = deal_hands()
            result, end_wager = play_hand(user_hand, dealer_hand, wager, bank) 
            if result == 'BJ':
                bank += (wager * 1.5)
            if result == 'Win':
                bank += end_wager
